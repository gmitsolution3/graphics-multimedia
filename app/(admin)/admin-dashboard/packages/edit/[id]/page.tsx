...
import EditPackageLoader from "@/components/loaders/EditPackageFormLoader";
...
export default function EditPackagePage({ params }: IProps) {
  const { id } = use(params);
  const router = useRouter();

  const { data: packageData, isLoading: isPackageLoading } =
    useGetById("/packages", id);
  const { data: servicesData, isLoading: isServicesLoading } =
    useGetServices();

  const { updateItem } = usePatch("/packages");

  const packageItem = packageData?.data || {};
  ...
  useEffect(() => {
    if (packageItem && Object.keys(packageItem).length > 0) {
      form.reset({
        ...
        period: packageItem.period || "monthly",
        cta: packageItem.cta || "Get Started",
  ...
  useEffect(() => {
    if (services.length > 0 && packageItem.services) {
      const existingServicesMap = new Map(
        packageItem.services.map((s: any) => [s.name, s.included]),
      );

      const mergedServices = services.map((service: any) => ({
        name: service.name,
        included: existingServicesMap.get(service.name) || false,
      }));
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="text-center py-12 px-4 border-2 border-dashed rounded-lg">
          ...
          <p className="text-gray-600 mb-6">
            The package you're trying to edit doesn't exist or has
            been removed.
          </p>
          <Button
            onClick={() => router.push("/admin-dsahboard/packages")}
          >
            Go Back to Packages
          </Button>
        </div>
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Update the details of your package. Make your changes below.
        </p>
      </div>
        ...
        >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Edit the basic details of your package including name,
