    ...
    <>
      ...
      >
        ...
        <Swiper
          ...
          loop
          speed={700} // 🔥 reduced from 1000
          allowTouchMove={false}
          onSwiper={(s) => (swiperRef.current = s)}
          onSlideChange={(s) => {
            setActiveIndex(s.realIndex); // ❌ removed animKey update
          }}
          className="absolute inset-0 w-full h-full"
        >
          {slides.map((sl, idx) => (
            <SwiperSlide key={idx} className="w-full h-full">
              <div key={`kb-${idx}`} className="kb absolute inset-0">
                <Image
                  src={sl.image}
                  alt={sl.imageAlt}
                  fill
...
"
        ...
        <div className="absolute inset-0 z-20 flex items-center">
          <div className="container mx-auto px-6 lg:px-16">
            <div className="text-center w-full md:text-left md:max-w-xl">
              {/* Tag */}
              ...
              <div className="a-cta flex flex-col items-center gap-3 sm:flex-row md:items-start">
                <Button
                  className="py-5 md:py-6 w-full max-w-xs md:w-50 uppercase"
                  asChild
                >
        ...
        <div className="absolute bottom-8 md:bottom-16 left-0 right-0 z-20">
          <div className="container mx-auto px-6 lg:px-16">
            <div className="flex flex-col items-center gap-4 md:flex-row md:items-start md:justify-between">
              {/* Stats */}
              <div className="a-stats flex justify-center lg:justify-start gap-6 md:gap-10 w-full overflow-x-auto md:overflow-visible pb-2 md:pb-0 scrollbar-hide">
                {slides[activeIndex].stats.map((stat) => (
                  <div
                    key={stat.label}
                    className="group flex-shrink-0 text-center md:text-left"
                  >
                    ...
                    <div className="w-4 md:w-6 h-px bg-primary/30 mx-auto md:mx-0 mt-1 md:mt-2 group-hover:w-8 transition-all duration-300"></div>
                  </div>
                ))}
              </div>
        ...
        <>
          <div className="hidden md:block absolute top-12 left-12 w-12 h-12 border-l border-t border-white/10 z-20"></div>
          <div className="hidden md:block absolute bottom-12 right-12 w-12 h-12 border-r border-b border-white/10 z-20"></div>
        </>
      </section>
    </>
  );
